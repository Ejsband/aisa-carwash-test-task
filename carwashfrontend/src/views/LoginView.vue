<template>
  <div class="login-contain wight">
    <section class="login" id="login">
      <h2>Автомойка</h2>
      <p ref="warningRef" class="error_1" v-html="message"></p>
      <div class="login-form">
        <input ref="loginRef" v-model="userName" type="text" class="login-input" placeholder="Пользователь" required
               autofocus/>
        <input ref="passwordRef" v-model="userPassword" type="password" class="login-input" placeholder="Пароль"
               required/>
        <div class="submit-container">
          <div @click="login()" class="login-button">ВОЙТИ</div>
        </div>
      </div>
    </section>
  </div>
...
export default {
  ...
  methods: {
    login() {
      ...
      if (login === '' || password === '') {
        this.$refs.warningRef.style.display = 'block'
        this.message = "введите данные"
      } else {
        http.get(`users/${login}`)
            .then((response) => {
              if (response.status === 200) {
                if (response.data.userPassword !== password) {
                  this.$refs.warningRef.style.display = 'block'
                  this.message = "неверный пароль"
                } else {
                  localStorage.setItem('role', response.data.userRole)
                  localStorage.setItem('login', "true")
                  this.$store.state.user = response.data.name
